...
namespace Hotel720.Platform.Infrastructure.Data.NoSql
{
	...
	/// </summary>
	public static class MongoDbConnection<U>
	{
		/// <summary>
		...
		public static IMongoCollection<T> GetCollectionFromConnectionString<T>(string connectionString)
			where T : IEntity<U>
		{
			return MongoDbConnection<U>.GetCollectionFromConnectionString<T>(connectionString, GetCollectionName<T>());
		}
		...
		public static IMongoCollection<T> GetCollectionFromConnectionString<T>(string connectionString, string collectionName)
			where T : IEntity<U>
		{
			return MongoDbConnection<U>.GetDatabaseFromUrl(new MongoUrl(connectionString))
		...
		public static IMongoCollection<T> GetCollectionFromUrl<T>(MongoUrl url)
			where T : IEntity<U>
		{
			return MongoDbConnection<U>.GetCollectionFromUrl<T>(url, GetCollectionName<T>());
		}
		...
		public static IMongoCollection<T> GetCollectionFromUrl<T>(MongoUrl url, string collectionName)
			where T : IEntity<U>
		{
			return MongoDbConnection<U>.GetDatabaseFromUrl(url)
				.GetCollection<T>(collectionName);
		}
